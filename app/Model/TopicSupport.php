...
class TopicSupport extends Model {
    ...
    public static function traverseChildTree($algorithm,$topicnum,$campnum,$delegateNickId,$parent_support_order,$multiSupport,$delegate_tree=[]){
        ...
        foreach($delegatedSupports as $support){ 
            if($support->camp_num == $campnum){              
                    $delegateTree = $delegate_tree[$support->nick_name_id];
                    $delegate_score = $camp->getDelegatesScore($delegateTree);
                    $array[$support->nick_name_id]['index']=$support->nick_name_id;
                    $array[$support->nick_name_id]['score'] =$delegateTree['score'];
                    $delegate_arr = $delegate_tree[$support->nick_name_id]['delegates'];
                    $array[$support->nick_name_id]['children'] = self::traverseChildTree($algorithm,$topicnum,$campnum,$support->nick_name_id,$parent_support_order,$multiSupport,$delegate_arr);
                }  
            }
    ...
    public static function traverseTree($algorithm,$topicnum,$campnum,$delegateNickId=0){
        ...
        foreach($supports as $key =>$support){
            ...
            if($currentCampSupport){
                ...
                $array[$support->nick_name_id]['children'] = self::traverseChildTree($algorithm,$topicnum,$campnum,$support->nick_name_id,$support_order,$multiSupport,$delegate_tree);
    ...
    // public static function traverseTree($algorithm,$topicnum,$campnum,$delegateNickId=0){
    //     self::traverseTreeLatest($algorithm,$topicnum,$campnum,$delegateNickId=0);
    //     $as_of_time = time();
	// 	if(isset($_REQUEST['asof']) && $_REQUEST['asof']=='bydate'){
	// 		$as_of_time = strtotime($_REQUEST['asofdate']);
	// 	}
        ...
        foreach($traversedTreeArray as $array){
            ...
            if($as_of_time < time()){
                if(in_array($array['index'],$userNicknames)){
                    $remove_support=TRUE;
                    $html.="<a data-toggle='tooltip' data-original-title='History cannot be modified. In order to modify your current support select the default option in the \"As Of\" box' style='".$disabledCss."' href='javascript:void(0)' class='btn btn-info singleClick disable-btn'>Remove Your Support </a>";
                }                
                ...
                if($remove_support ==FALSE && Auth::check() && !in_array($array['index'],$userNicknames) && !in_array($array['index'],$myDelegator) && !in_array($array['index'],$myDelegation)){
                    ...
                    $html.="<a data-toggle='tooltip' data-original-title='History cannot be modified. In order to modify your current support select the default option in the \"As Of\" box' style='".$disabledCss."' href='javascript:void(0)' class='btn btn-info singleClick disable-btn'>Delegate Your Support </a>";
                }
            }else{
                if(in_array($array['index'],$userNicknames)){
                    $remove_support =TRUE;
