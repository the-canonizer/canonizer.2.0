                ...
                @if(count($supportedTopic))
                 ...
                 <?php $title      = preg_replace('/[^A-Za-z0-9\-]/', '-', $data->topic->topic_name);	?>
					   ...
					   <div id="positions_{{ $support->support_id }}" class="SpCmpBDY support-sorter-element ui-widget ui-widget-content ui-helper-clearfix ui-corner-all">
					     ...
					     <b><span class="support_order">{{ $support->support_order }}</span> . <a style="text-decoration: underline; color: blue;" href="/topic/{{ $data->topic_num }}-{{ $title }}/{{ isset($support->camp) ? $support->camp->camp_num : 1 }}"> {{ isset($support->camp) ? $support->camp->camp_name : ""}}  </a> <br/>
					   	 </b>
                       </div>
						<?php } else { 
