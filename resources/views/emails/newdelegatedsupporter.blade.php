@component('mail::message')
 ...
 @if (isset($data['support_added']) && $data['support_added'] == 1 )
 	{{ $data['nick_name']}} has just added their support to this topic: <a href="{{ url('/') . '/' . $link }}"><b>{{ $data['object']}}</b></a>
 @elseif(isset($data['support_deleted']) && $data['support_deleted'] == 1)
 {{ $data['nick_name']}} has just deleted their support to this topic: <a href="{{ url('/') . '/' . $link }}"><b>{{ $data['object']}}</b></a>
 @else
